...
const eikaiwaTable = process.env.EIKAIWA_TABLE
...
function addNewArticle(event, context, callback) {
...
function getAllArticles(event, context, callback) {
  ...
  return (
    dataBase
      ...
      .scan({
        TableName: eikaiwaTable, //which table to scan
      })
...
function getArticleById(event, context, callback) {
...
function updateArticleCommentsById(event, context, callback) {
  // Gets the id out of the parameters of the event aka the request (the equivalent of doing req.params).
